...
*
*   pauli_wsp_t *alloc_pauli_wsp(void)
*     Returns the address of a new instance of an pauli_wsp_t workspace.
*     A NULL pointer is returned if the allocation fails.
*
*   void free_pauli_wsp(pauli_wsp_t *pwsp)
*     Frees the pauli_wsp_t workspace swsp previously allocated by
*     alloc_pauli_wsp().
*
*   int inv_pauli_dble(double mu,pauli_dble *m,pauli_wsp_t *pwsp,
*                      pauli_dble *im)
*     Computes the Hermitian part of the matrix (m+i*mu)^(-1), using the
*     arrays in the structure pwsp as workspace, and assigns the computed
*     matrix to im. The matrix m is overwritten if im=m and otherwise left
*     unchanged. On exit the program returns 0 or 1 depending on whether
*     the inversion was safe or not (in which case the calculated matrix
*     is unusable).
*
*   complex_dble det_pauli_dble(double mu,pauli_dble *m,pauli_wsp_t *pwsp)
*     Computes the determinant of the matrix m+i*mu, using the arrays in
*     the structure pwsp as workspace, and returns the computed value.
*
...
*     is overwritten if r=s and otherwise left unchanged.
*
*   int apply_swinv_dble(int vol,double mu,pauli_dble *m,spinor_dble *s,
*                        pauli_wsp_t *pwsp,spinor_dble *r)
*     Applies the inverse of the matrix field m[2*vol]+i*mu*gamma_5 to
*     the spinor field s[vol], using the arrays in the structure pwsp as
*     workspace, and assigns the result to the field r[vol]. The source
*     field is overwritten if r=s and otherwise left unchanged. On exit
*     the program returns 0 or 1 depending on whether the inversions were
*     safe or not (in the latter case, the output field is unusable).
*
...
* (M+M^dag)/2.
*
* The program apply_swinv_dble() performs matrix inversions on the fly and is
* therefore much slower than apply_sw_dble(). When calling inv_pauli_dble()
* or apply_swinv_dble(), it is up to the calling program to check the return
* value and to take the appropriate action.
*
* The type pauli_wsp_t is defined in the file include/sw_term.h.
*
* The programs alloc_pauli_wsp() and free_pauli_wsp() are assumed to be
* called by the OpenMP master thread. They perform no communications and
* can be locally called. All other programs are thread-safe.
*
* If SSE (AVX) instructions are used, the Pauli matrices, Dirac and Weyl
* spinors must be aligned to a 16 (32) byte boundary.
*
*******************************************************************************/
...
static int fwd_house(double eps,pauli_wsp_t *pwsp)
{
   int i,j,k,ifail;
   double r1,r2,r3,*rr;
   complex_dble z,*ak,*aj,*aa,*dd;
...
static void solv_sys(pauli_wsp_t *pwsp)
{
   int i,j,k;
   complex_dble *aa,*dd;
...
static void bck_house(pauli_wsp_t *pwsp)
{
   int i,j,k;
   double *rr;
   complex_dble z,*d,*a,*aa,*dd;
...
pauli_wsp_t *alloc_pauli_wsp(void)
{
   ...
   pwsp=malloc(sizeof(*pwsp));
   rr=malloc(8*sizeof(*rr));
   aa=amalloc(48*sizeof(*aa),5);

   if ((pwsp==NULL)||(rr==NULL)||(aa==NULL))
      return NULL;
...
static double set_aa(double mu,pauli_dble *m,pauli_wsp_t *pwsp)
{
   int i,j;
   double sm,*u,*v;
   complex_dble *aa;
...
static double norm_aa(pauli_wsp_t *pwsp)
{
   double sm;
   complex_dble *aa,*am;
   ...
   for (;aa<am;aa++)
      sm+=(*aa).re*(*aa).re+(*aa).im*(*aa).im;

   return sm;
}


static void apply_aa(complex_dble *v,pauli_wsp_t *pwsp,complex_dble *w)
{
   complex_dble *aa,*cc;
...
int inv_pauli_dble(double mu,pauli_dble *m,pauli_wsp_t *pwsp,pauli_dble *im)
{
   int i,j,ifail;
   double eps,sm,*u,*v;
   complex_dble *z,*w,*aa;

   eps=DELTA*set_aa(mu,m,pwsp);
   ifail=fwd_house(eps,pwsp);
   solv_sys(pwsp);
   bck_house(pwsp);
...
complex_dble det_pauli_dble(double mu,pauli_dble *m,pauli_wsp_t *pwsp)
{
   int i,j,k;
   double eps,r1,r2,r3;
   complex_dble det,z,w,*aa;

   eps=DBL_EPSILON*sqrt(set_aa(mu,m,pwsp));
   det.re=1.0;
   det.im=0.0;
   aa=(*pwsp).aa;
                     ...
                     pauli_wsp_t *pwsp,spinor_dble *r)
{
   ...
   for (;ps<pm;ps++)
   {
      ...
      solv_sys(pwsp);
      bck_house(pwsp);
      if ((eps*norm_aa(pwsp))>1.0)
         ifail=1;
      apply_aa((*ps).c,pwsp,(*pr).c);
      m+=1;
